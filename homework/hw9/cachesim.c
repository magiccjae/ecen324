...
#define BLOCKSIZE    2  // size in bytes
 ...
 * to list them in hex.
 */
int ref[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x40, 0x42, 0x41 } ;
...
int main()
{
	...
	{
		if(cache[index][i].valid == VALID && cache[index][i].tag == tag)
		{
			outcome = HIT;
	...
	if (outcome == MISS)
	{
	    ...
	    cache[index][min].valid = VALID;
	 	cache[index][min].reference = refnum;
	}
